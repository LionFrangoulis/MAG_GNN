...
def analyse_angular_model(size, run, model,train_loader, test_loader, save_on_disc=False):
    # Loop over the test set
    y_train=torch.tensor([])
    y_pred_train=torch.tensor([])
    identifiers_train=[]
    with torch.no_grad():  # Disable gradient calculation for testing
        for batch_data in train_loader:
            rbf_train_batch, sbf_train_batch, elements_train, y_train_batch, identifiers_train_batch, ligands, train_atom_numbers = batch_data
            mask=torch.tensor([[1]*atom_number+[0]*(train_atom_numbers[-1]-atom_number) for atom_number in train_atom_numbers]).to(device)
            identifiers_train=np.concatenate([identifiers_train,identifiers_train_batch])
            rbf_train_batch=rbf_train_batch.to(device)
            sbf_train_batch=sbf_train_batch.to(device)
            y_train_batch=y_train_batch.to(device)
            y_pred_train_batch = model(elements_train,rbf_train_batch, sbf_train_batch,train_atom_numbers[-1], mask)
            y_train=torch.cat([y_train, y_train_batch])
            y_pred_train=torch.cat([y_pred_train, y_pred_train_batch])
    ...
    with torch.no_grad():  # Disable gradient calculation for testing
        for batch_data in test_loader:
            rbf_test_batch, sbf_test_batch, elements_test, y_test_batch, identifiers_test_batch, ligands, test_atom_numbers = batch_data
            mask=torch.tensor([[1]*atom_number+[0]*(test_atom_numbers[-1]-atom_number) for atom_number in test_atom_numbers]).to(device)
            identifiers_test=np.concatenate([identifiers_test,identifiers_test_batch])
            rbf_test_batch=rbf_test_batch.to(device)
            sbf_test_batch=sbf_test_batch.to(device)
            y_test_batch=y_test_batch.to(device)
            y_pred_test_batch = model(elements_test,rbf_test_batch,sbf_test_batch, test_atom_numbers[-1], mask)
            y_test=torch.cat([y_test, y_test_batch])
            y_pred_test=torch.cat([y_pred_test, y_pred_test_batch])
...
def load_train_dataset(blocks, filter_number, sbf_cutoff, max_angle, rbf_location, sbf_location, rbf_cutoff, load_sbf=True, load_rbf=True):
    '''
    ...
    dataset=[]
    for block in blocks:
        atom_numbers=np.loadtxt("{}/Train_Block_{}_atom_numbers.txt".format(rbf_location,block), dtype="int")
        elements=np.load("{}/Train_Block_{}_elements.npy".format(rbf_location,block))
        energies=np.loadtxt("{}/Train_Block_{}_energies.txt".format(rbf_location,block))
        identifiers=np.loadtxt("{}/Train_Block_{}_identifiers.txt".format(rbf_location,block), dtype="int")
        ligands=np.loadtxt("{}/Train_Block_{}_ligands.txt".format(rbf_location,block), dtype="str")
        if load_rbf:
            rbf=torch.tensor(np.load("{}/Train_Block_{}_Filters_number_{}_cutoff_{}.npy".format(rbf_location,block, filter_number,rbf_cutoff)),dtype=torch.float32)
            if load_sbf:
                sbf=torch.tensor(np.load("{}/Train_Block_{}_atomic_sbf_cutoff_{}_max_angle_{}_L_7.npy".format(sbf_location,block,sbf_cutoff,max_angle)),dtype=torch.float32)
                #sbf=torch.tensor(np.load("{}/Test.npy".format(sbf_location)))
                dataset.append((rbf, sbf, elements, torch.tensor(energies,dtype=torch.float32), identifiers, ligands, atom_numbers))
            else:
                dataset.append((rbf, elements, torch.tensor(energies,dtype=torch.float32), identifiers, ligands, atom_numbers))
        else:
            if load_sbf:
                sbf=torch.tensor(np.load("{}/Train_Block_{}_atomic_sbf_cutoff_{}_max_angle_{}_L_7.npy".format(sbf_location,block,sbf_cutoff,max_angle)),dtype=torch.float32)
                dataset.append((sbf, elements, torch.tensor(energies,dtype=torch.float32), identifiers, ligands, atom_numbers))
            else:
                dataset.append((elements, torch.tensor(energies,dtype=torch.float32), identifiers, ligands, atom_numbers))
    return(dataset)

def load_test_dataset(blocks, filter_number, sbf_cutoff, max_angle, rbf_location, sbf_location, rbf_cutoff, load_sbf=True, load_rbf=True):
    '''
    All info used as features are previously generated and stored. This then load in those info for the model.
    QUESTION IS WHY IS DATA SPLITTED IN TRAIN AND TEST AND LATER LOADED INTO THE MODEL? 
    '''
    dataset=[]
    for block in blocks:
        atom_numbers=np.loadtxt("{}/Test_Block_{}_atom_numbers.txt".format(rbf_location,block), dtype="int")
        elements=np.load("{}/Test_Block_{}_elements.npy".format(rbf_location,block))
        energies=np.loadtxt("{}/Test_Block_{}_energies.txt".format(rbf_location,block))
        identifiers=np.loadtxt("{}/Test_Block_{}_identifiers.txt".format(rbf_location,block), dtype="int")
        ligands=np.loadtxt("{}/Test_Block_{}_ligands.txt".format(rbf_location,block), dtype="str")
        if load_rbf:
            rbf=torch.tensor(np.load("{}/Test_Block_{}_Filters_number_{}_cutoff_{}.npy".format(rbf_location,block, filter_number, rbf_cutoff)),dtype=torch.float32)
            if load_sbf:
                sbf=torch.tensor(np.load("{}/Test_Block_{}_atomic_sbf_cutoff_{}_max_angle_{}_L_7.npy".format(sbf_location,block,sbf_cutoff,max_angle)),dtype=torch.float32)
                dataset.append((rbf, sbf, elements, torch.tensor(energies,dtype=torch.float32), identifiers, ligands, atom_numbers))
            else:
                dataset.append((rbf, elements, torch.tensor(energies,dtype=torch.float32), identifiers, ligands, atom_numbers))
        else:
...
def train_angular_SchNet(feature_number, element_number, rbf_number, sbf_number, layer_number, epochs, train_loader, test_loader, start_learning_rate,pooling, factor=0.5, patience=3, threshold=1e-4, min_lr=1e-6):
    history = []
    model = GNN.SchNet_Atomic_Angles(feature_number, element_number, rbf_number, sbf_number, layer_number, pooling, ghost_killer=True).to(device)
    ...
    for epoch in range(epochs):
        ...
        losses=[]
        for batch_data in train_loader:
            rbf_train, sbf_train, elements_train, y_train, identifiers, ligands, atom_numbers = batch_data
            mask=torch.tensor([[1]*atom_number+[0]*(atom_numbers[-1]-atom_number) for atom_number in atom_numbers]).to(device)
            #sbf_train=torch.zeros([500,atom_numbers[-1],atom_numbers[-1],30])
            rbf_train=rbf_train.to(device)
            sbf_train=sbf_train.to(device)
            
            y_train=y_train.to(device)
            y_pred = model(elements_train,rbf_train, sbf_train,atom_numbers[-1], mask)
            loss = dnn_loss(y_pred, y_train.view(-1))
            optimizer.zero_grad() 
            loss.backward()
            losses.append(loss)
            train_loss += loss.detach().item() * rbf_train.size(0)
            total_samples += rbf_train.size(0)  # Count samples in the batch
            optimizer.step() # update the parameters 
        avg_loss = train_loss / total_samples  # Normalize by number of batches
        scheduler.step(avg_loss)
        ...
        with torch.no_grad():
            for batch_data in test_loader:
                rbf_val, sbf_val, elements_val, y_val, _, _, atom_numbers = batch_data
                #sbf_val=torch.zeros([200,atom_numbers[-1],atom_numbers[-1],30])
                # x_val the RBF filters (N,F)
                # elements_val the elements in each mol (N,A)
                # atoms_no the original no of atoms in each mol (N)
                sbf_val=sbf_val.to(device)
                rbf_val=rbf_val.to(device)
                y_val=y_val.to(device)
                mask=torch.tensor([[1]*atom_number+[0]*(atom_numbers[-1]-atom_number) for atom_number in atom_numbers]).to(device)
                #mask=None
                yval_pred = model(elements_val,rbf_val, sbf_val,atom_numbers[-1], mask) # the last one has the largest size 
                loss = dnn_loss(yval_pred, y_val.view(-1))
                val_losses.append(loss)
                val_loss += loss.detach().item() * rbf_val.size(0)
                total_val_samples += rbf_val.size(0)  # Count samples in the batch
        avg_val_loss = val_loss / total_val_samples  # Normalize by number of batches
        history.append([epoch,avg_loss, avg_val_loss])
...
if __name__=="__main__":
    ...
    rbf_location="/home/lion/Documents/GNN_Clean/Data/Dy_Relaxed_Block_Data/"
    ...
    layer_number=4#Modifiable
    LR=0.0025
    epochs=50
    max_angle=7
    
    train_loader = load_train_dataset(range(block_number),filter_number,3,max_angle,rbf_location,sbf_location, rbf_cutoff)
    test_loader = load_test_dataset(range(10),filter_number,3,max_angle,rbf_location,sbf_location, rbf_cutoff)
    
    
        
    
    
    history, angular_model=train_angular_SchNet(feature_number, element_number, filter_number, sbf_number, layer_number, epochs, train_loader, test_loader, LR, "mean")
    ...
    np.savetxt(output_folder_dir+"history_layer_number_{}_feature_number_{}_block_number_{}_run_{}_rbf_number_{}_sbf_number_{}.pyt".format(layer_number,feature_number,block_number,run,filter_number, sbf_number),history)
    torch.save(angular_model.state_dict(), output_folder_dir+"model_weights_layer_number_{}_feature_number_{}_block_number_{}_run_{}_rbf_number_{}_sbf_number_{}.pyt".format(layer_number,feature_number,block_number,run,filter_number, sbf_number))
    plot_loss_growth(history)
    analyse_angular_model(feature_number,0,angular_model, train_loader, test_loader)
    #analyse_model(feature_number,0,model,train_loader, test_loader)
    #del(train_loader)
    #del(test_loader)
    print("DONE")
