...
class SchNet_Pure_Angles(nn.Module):
    def __init__(self, feature_number, element_number, sbf_number, layer_number, pooling, ghost_killer=True):
        super(SchNet_Pure_Angles, self).__init__()
        ...
        self.interaction_layers=nn.ModuleList([interaction(self.feature_number, sbf_number, ghost_killer) for i in range(self.layer_number)]) 
    ...
    def forward(self,elements,sbf, atom_number, mask=None):
        #Input dimensions elements: (N*A) scalar then?! 
        #Input dimension sbf: (N,A,A,L)
        
        x=self.encoding(elements) # output dim: (N*A,F)
        sbf=torch.reshape(sbf,[-1,atom_number, atom_number, self.sbf_number])
        for i in range(self.layer_number):
            x=self.interaction_layers[i](x,sbf, atom_number) # ATOM_NUMBER IS REDUNDANT
...
class cfconv(nn.Module):
    ...
    '''
    def __init__(self, feature_number,sbf_number,ghost_killer):
        super(cfconv, self).__init__()
        self.sbf_number=sbf_number
        self.feature_number=feature_number
        self.filter_layer_1=nn.Linear(self.sbf_number,self.feature_number, bias=not ghost_killer)
        #self.ssp=SSP()
        self.ssp=torch.nn.ELU()
        #self.layer_2=nn.Linear(self.feature_number,self.feature_number)
         
    def forward(self,x,sbf,atom_number):
        #Input dimension x: (N*A,F)
        #Input dimension sbf: (N,A,A,L)
        x=torch.reshape(x,(-1,atom_number, self.feature_number)) # dim: (N,A,F)
        #sbf=sbf.transpose(1,2)
        filter_inputs=torch.reshape(sbf,(-1,self.sbf_number))
...
class interaction(nn.Module):
    ...
    def __init__(self,feature_number,sbf_number,ghost_killer):
        ...
        self.cfconv1=cfconv(self.feature_number,self.sbf_number,ghost_killer)
        #self.ssp=SSP()
        self.ssp=torch.nn.ELU()
    ...
    def forward(self, x, sbf, atom_number):
        ...
        v=self.cfconv1(v,sbf,atom_number)  # NO OF ATOMS IS ONLY FOR FIXING THE DIM OF MATRICES. 
